...
import Chitava.SpringHW5.services.FileGateway;
...
public class WebController {
   ...
   public String createNote(){
    ...
    public String createNote (@ModelAttribute("annotation")String annotation, Note note, Model model){
        ...
        fileGateway.writeToFile("timecreate.txt", "Создали новую запись " + (LocalDateTime.now()));
        fileGateway.writeToFile("logger.txt", "Посещали страницу создания записок"+ LocalDateTime.now());
    ...
    public String allNotes(Model model) {
        ...
        fileGateway.writeToFile("logger.txt", "Получение всех записок "+ LocalDateTime.now());
        return "index";
    }
    ...
    public String deletById(@PathVariable Long id, Model model) {
        Note deleteNote = service.getById(id);
        service.deleteNode(id);
        fileGateway.writeToFile("logger.txt", "удаление записки c текстом " + deleteNote.getAnnotation()
